...
class MultiNavServer:
    ...
    def arm_status_callback(self, msg):
        ...
        if msg.arm_id < self._n_cars and msg.status:
            ...
            if self._cars[msg.arm_id].is_moving:
                ...
                rospy.logwarn("Car {} is moving, but arm is working, this should not happen!".format(msg.arm_id))
                return
            ...
            if self._cars[msg.arm_id].arm_picking:
                rospy.loginfo(
                    "Car {} finished picking, sending goal {}...".format(msg.arm_id, self._cars[msg.arm_id].curr_target))
                self._cars[msg.arm_id].arm_picking = False
                self._cars[msg.arm_id].activate_car()
                # The station where the car finished picking should be added to available stations list
                self._available_stations.append(self._jobs[msg.job]["transfer"]["start"])
            # The second call should be the arm finished dropping, so the next move is set the car to ready
                ...
                rospy.loginfo("Car {} finished dropping, setting car to ready...".format(msg.arm_id))
                ...
                rospy.loginfo("Car {} is available list".format(msg.arm_id))
                # The car finished transporting the object, then add the station to working stations list
                self._working_stations.append(self._jobs[msg.job]["transfer"]["end"])
                self._jobs[msg.job]["transfer"]["start"] = self._jobs[msg.job]["transfer"]["end"]
    ...
    def get_dispatch_routes(self, job):
        ...
        # Check if the start station is in the available stations list
        if start in self._available_stations:
            self._available_stations.remove(start)
        else:
            rospy.logwarn_throttle(1, "Start station {} is not available, returning -1".format(start))
            return -1, -1
